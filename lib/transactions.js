...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients){
    ...
    var scriptSigPart1 = Buffer.concat([
        ...
        //new Buffer("086b6f746f706f6f6c", 'hex'),
        util.serializeNumber(Date.now() / 1000 | 0),
        new Buffer([extraNoncePlaceholder.length])
    ]);
    ...
    // for Koto transaction v3 format 
    var nVersionGroupId = (txVersion & 0x7fffffff) >= 3 ? util.packUInt32LE(0x3c48270) : new Buffer([]);
    ...
    // for Koto transaction v2/v3 format
